...
pub const OperationTable = struct {
    ...
    accept: *const fn (ctx: *anyopaque) anyerror!net.Server.Connection,
        ...
        pub usingnamespace switch (contype) {
            .ClientConnectionType => blk: {
                if (is_wasm) break :blk struct {} else break :blk struct {
                    ...
                    pub fn write(self: *Self, data: []const u8) !void {
                        while (@atomicRmw(bool, &self.wlock, .Xchg, true, .seq_cst)) {} // two threads should write at the same time
            ...
            .ServerConnectionType => struct {
                ...
                pub fn accept(self: *Self) !*ClientConnectionType {
                    ...
                    if (is_websocket) {
                        const client = try WebConnection.init(self.allocator, conn.stream, addr, .{});
