...
pub fn startServer(_: [*c]PyObject, args: [*c]PyObject) callconv(.C) [*]PyObject {
    ...
    const handler_thread = std.Thread.spawn(.{}, Server.messageDispatcher, .{ allocator, papp_name, .{ .host = phost, .port = pport, .mode = pmode, .password = ppassword }, conn_num }) catch return Py_BuildValue("");
